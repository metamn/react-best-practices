...
By definition `const memoizedValue = useMemo(() => computeExpensiveValue(a, b), [a, b]);` returns a memoized _value_:
...
> React Hook "useData" cannot be called inside a callback. React Hooks must be called in a React function component or a custom React Hook function react-hooks/rules-of-hooks
...
The keywords here are referential equality.
...
1. Use `React.memo()`, `useCallback()` or `useMemo()` to memoize components, functions and values only when the performance measurements make it required.
2. Always `useCallback()` to memoize functions which will be used in referential equality checks.
3. Always `useMemo()` to memoize values which will be used in referential equality checks.
