...
Different images are displayed on landscape and portrait screens.
...
In config a default filter is set to `q35.foil1`. That means quality=35 which in text screenshots is to low and needs to be adjusted indiviually.
...
Both the image quality and size was increased. The original image size is 231kb while this has 100kb &mdash; and stays still readable.
